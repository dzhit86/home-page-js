...
if (sessionStorage.getItem('formText') != null && !$('*').is('.master__blockReviewItem-reviews') && window.location.href.includes("/therapist/")) {
...
Autocomplete('.section__loginInput-cities', true);
function Autocomplete (selector, linkMode = false) {
                ...
                data.forEach((dataItem, index) => {
                  ...
                  let item;
                    if (!linkMode) {
                        ...
                        item.className = 'autocomplete-item';
                        item.setAttribute("href", `/${placesSlug}/`);
                        item.innerHTML = parts[0] + '<strong>' + parts[1] + '</strong>' + parts[2];
                        list.appendChild(item);
                        listItems.push(item);
                    }
...
// Verified Label
const verifiedImages = document.querySelectorAll("._verified-photo");
verifiedImages.forEach(element => {
    element.style.position = "relative";
    let label = document.createElement("div");
    label.insertAdjacentHTML("afterBegin", `<span class="_verified-photo__icon">✓</span>`);
    label.insertAdjacentHTML("beforeEnd", `<span class="_verified-photo__text">Verified Account</span>`);
    label.classList.add("_verified-photo__label")
